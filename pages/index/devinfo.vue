...
<script>
	export default {
		data() {
			return {
				devid:this.globalVal.device_info.devid,
				devinfo:null,
				userinfo:null,
				termperature:null,
				humidity:null,
		...
		methods:{
			...
			getInfo(){
				...
				uni.request({//向云端服务发送请求获取设备最新信息
					...
					success: res => {
						console.log("url: ", this.globalVal.default_url.devInfo);
						console.log(res);
						if(200 == res.statusCode){
							...
							if(undefined == res.data.error_code){
								...
								if(null != this.userinfo) {
									...
									console.log("no infostring received!");
								}
					...
					fail: () => {
						console.log("get_info fail!");
					},
					complete: () => {
						console.log("get_info complete");
						uni.hideLoading();
					}
				});
			},
